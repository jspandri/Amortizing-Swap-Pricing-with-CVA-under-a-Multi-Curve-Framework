function [price, price_clean, CVA] = price_swap_CVA(a, sigma, K, dt, ...
    ...
    payment_dates, yf, fixing_start, fixing_end, notional_amortized, RecoveryRate, HazardRate)
...
%   CVA                : [Scalar] Credit Value Adjustment computed via backward induction.
    ...
    for i = N_steps:-1:1
        ...
        % Identify the current grid time
        t_curr = grid_dates(i);
        
        % Identify the next grid time (looking forward in time, backward in induction)
        ...
        % Inject relevant cash flows (fixed or float) into the V vector if 
